...
'''
def curvature(in_array, fit_paras):
...
def curvature_meter(in_array, fit_paras, ym_per_px = 30/720, xm_per_px = 3.7/700):
    y_eval = np.max(in_array)
...
def updateFitParameters(primary_in, secondary_in, current_out, previous_out, p_previous_out):
    ...
    if normal_flag == True:
        ...
        print('Use old value')
        current_out = 0.5*previous_out + 0.1*p_previous_out + 0.4*primary_in
        previous_out = current_out
        p_previous_out = previous_out
...
#    print('normal flag is: ' + normal_flag)
    ...
    left_fitx = left_fit_updated[0]*ploty**2 + left_fit_updated[1]*ploty + left_fit_updated[2]
    right_fitx = right_fit_updated[0]*ploty**2 + right_fit_updated[1]*ploty + right_fit_updated[2]
...
#    filtered_fit_r = instance_line_right.current_fit
    ...
    # 5. CURVATUREs
    curvature_m_left, curvature_m_right = curvature_meter(ploty, fit_paras)
...
#        curvature_m_est = 0.5 * (instance_line_left.radius_of_curvature + instance_line_right.radius_of_curvature)
    ...
    # 6. MERGE IMAGE & LANEs
    lane_shaded = warpback(undistort, warped_img, Minv, ploty, left_fitx, right_fitx)
    offset_m = offset(undistort, left_fitx, right_fitx)
