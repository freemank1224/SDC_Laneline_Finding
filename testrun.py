...
def thresholdingBinary(img, s_thresh=(170, 250), sx_thresh=(100, 255), sy_thresh=(100,255), dir_thresh=(-0.6,-0.9), mag_thresh=(30,200)):
    img = np.copy(img)
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    print(gray.shape)
...
#                normal_flag = True
                ...
                instance_line_left.best_curvature[-1] = 0.5*curvature_m_left +  0.4*instance_line_left.best_curvature[-1] + 0.1*instance_line_left.best_curvature[-2]
##                normal_flag = False
#                blind_search_flag = 1
...
#                normal_flag = True
                ...
                instance_line_right.best_curvature[-1] = 0.5*curvature_m_right + 0.4*instance_line_right.best_curvature[-1] + 0.1*instance_line_right.best_curvature[-2]
##                normal_flag = False
#                blind_search_flag = 1
    else:
...
#    instance_line_left.radius_of_curvature = np.mean(instance_line_left.best_curvature)
#    instance_line_right.radius_of_curvature = np.mean(instance_line_right.best_curvature)
...
#        curvature_m_est = 0.5 * (instance_line_left.radius_of_curvature + instance_line_right.radius_of_curvature)
    ...
    if abs(offset_m) > 0.5:
        ...
        cv2.putText(lane_shaded, display_deviation, (250,100), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), thickness = 2)
